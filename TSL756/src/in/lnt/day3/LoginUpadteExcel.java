...
public static void main(String[] args) throws Exception {
			...
			for (int i = 0; i <= sheet.getLastRowNum(); i++) {
				...
				if (driver.getTitle().equals("Oracle")) {
					sheet.getRow(i).createCell(3).setCellValue("Login sucessful");
					...
					sheet.getRow(i).createCell(3).setCellValue("Login not sucessful");
					System.out.println("login fail");
					
					System.out.println("--------------------------------------------------------------------------");
				}
				FileOutputStream fileOut = new FileOutputStream("D:\\LoginStatus.xlsx");
				book.write(fileOut);
			}
